# Three-Player Crew Reference

| Role | Player Skill | Station |
|------|-------------|---------|
| Supreme Commander | Presence (Leadership) | Command Bridge + Sensor Station |
...
The Supreme Commander tab manages crew logistics, damage control, void shields, sensor locks, and the **Standing Orders** deck - a hand of tactical order cards drawn each round.
...
After the Gunnery Officer fires, the Supreme Commander may perform one BDA correction:
...
18-card deck shuffled at the start of combat. Draw 3 cards per round. Hand cap is 6. One free redraw per round (swap one card for a fresh draw). Spending two repair actions unlocks a full mulligan.
...
The Engineer also steers the ship using the **Helm Control** panel, with a live canvas overlay showing the projected path, turning arc, and minimum-move zone. Helm allocation rolls use **Tech (Engineering)**.

Roll Tech (Engineering) once per round. Allocate SL to:
...
Commit thrust using the power bar slider (0–100%). Auxiliary Power can be diverted to extend the power bar maximum.
